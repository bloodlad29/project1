...
for key,value in departures.items():
    s[key] = []
    for key1, value1 in tours.items():
        #print(departures[i])
        if key == value1["departure"]:
            s[key]+=[value1["price"]]
p = {}
for key,value in departures.items():
    p[key] = []
    for key1, value1 in tours.items():
        ...
        if key == value1["departure"]:
            p[key]+=[value1["nights"]]
s={'msk': [62000, 68000, 52000], 'spb': [53000, 68000, 82000, 74000], 'nsk': [62000, 63000, 39000], 'ekb': [85000, 72000, 49000], 'kazan': [44000, 72000, 91000]}
@app.route("/")
def main():

    return render_template("main.html", tours = tours, title = title, subtitle = subtitle, description = description, departures= departures, s = s)


@app.route("/tour/")
def tour():
    return render_template("tour.html", tours = tours, title = title, subtitle = subtitle, description = description, departures= departures)

@app.route("/departure/")
def departure():
    return render_template("departure.html", tours = tours, subtitle = subtitle, description = description, departures= departures, s = s, p = p)
